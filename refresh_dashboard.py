...
PLUGS = ['AFib', 'Cardio', 'CnO Pro', 'Respiratory', 'Tesla', 'Migraine', 'BetterHelp', 'GLP1']
...
def fetch_user_data():
    ...
    # ---- Build output ----
    user_data = {}
    grand_male, grand_female, grand_other, grand_total_g = 0, 0, 0, 0
    for pp in PLUGS:
        ...
        grand_other += gd['other']
        grand_total_g += gd['total']
        print(f"    {pp}: {total_active:,} active ({paid:,} paid + {on_trial:,} on trial, {male_pct}%M/{female_pct}%F/{other_pct}%O)")
    ...
    user_data['_total'] = {
        ...
        'other': round(grand_other / ggt * 100),
    }
...
def fetch_country_user_data():
    ...
    Returns dict: { country: { pp: { paid, on_trial, users, male%, female%, other% } } }
    """
...
def _hardcoded_user_data():
    ...
    return {
        'AFib':        {'users': 3700,  'paid': 3700,  'on_trial': 0, 'male': 62, 'female': 35, 'other': 3},
        'Cardio':      {'users': 34211, 'paid': 34211, 'on_trial': 0, 'male': 58, 'female': 39, 'other': 3},
        'CnO Pro':     {'users': 24835, 'paid': 24835, 'on_trial': 0, 'male': 0, 'female': 95, 'other': 5},
        'Respiratory': {'users': 7288,  'paid': 7288,  'on_trial': 0, 'male': 51, 'female': 46, 'other': 3},
        'Tesla':       {'users': 99,    'paid': 99,    'on_trial': 0, 'male': 65, 'female': 32, 'other': 3},
        '_total':      {'users': 0, 'male': 57, 'female': 40, 'other': 3},
    }
...
# ============================================================
# FETCH GLP-1 DATA (separate MySQL DB via Metabase cards)
# ============================================================
GLP1_DAILY_REVENUE_CARD = 20321   # Daily revenue USD from glp1_orders
GLP1_REGION_REVENUE_CARD = 20323  # Region-wise revenue (India/UAE)
GLP1_DAILY_SIGNUPS_CARD = 20302   # Daily signups from glp1_profiles
GLP1_DAILY_ORDERS_CARD = 20317    # Daily orders placed
GLP1_ACTIVE_COUNTRIES = ['USA', 'India', 'UAE']

# ── Migraine constants ──
MIGRAINE_ACTIVE_COUNTRIES = ['India', 'UAE', 'USA', 'UK', 'Germany', 'Czech Republic', 'Spain', 'Netherlands', 'Switzerland', 'France', 'Poland', 'Austria', 'Italy', 'Belgium']
MIGRAINE_SUBSCRIPTION_PRICE_USD = 5.99
...
def fetch_glp1_data():
    ...
    Returns dicts that can be merged into revenue_data, country_revenue,
    purchase_data, and trial_data.
    """
    print("\n--- Fetching GLP-1 data ---")

    # 1. Fetch daily revenue
    print("  Fetching GLP1 daily revenue (card 20321)...")
    try:
        result = mb_post(f'card/{GLP1_DAILY_REVENUE_CARD}/query')
        rows = result.get('data', {}).get('rows', [])
        print(f"    Got {len(rows)} daily revenue rows")
    except Exception as e:
        print(f"    ERROR fetching GLP1 daily revenue: {e}")
        rows = []

    # Parse daily revenue into month-keyed structure
    glp1_revenue = {}  # { "2026-01": { dates: [], revenue: {GLP1: [...]}, subscriptions: {GLP1: [...]} } }
    for row in rows:
        date_str = str(row[0])[:10]  # revenue_date
        rev = float(row[1] or 0)     # daily_revenue_usd
        orders = int(row[2] or 0)    # paid_orders
        month_key = date_str[:7]

        if month_key not in glp1_revenue:
            glp1_revenue[month_key] = {'dates': [], 'revenue': [], 'orders': []}
        glp1_revenue[month_key]['dates'].append(date_str)
        glp1_revenue[month_key]['revenue'].append(round(rev, 2))
        glp1_revenue[month_key]['orders'].append(orders)

    # 2. Fetch region-wise revenue for country breakdown
    print("  Fetching GLP1 region revenue (card 20323)...")
    try:
        result = mb_post(f'card/{GLP1_REGION_REVENUE_CARD}/query')
        region_rows = result.get('data', {}).get('rows', [])
        print(f"    Got {len(region_rows)} region rows")
    except Exception as e:
        print(f"    ERROR fetching GLP1 region revenue: {e}")
        region_rows = []

    # Map region names to dashboard country names
    region_map = {'India': 'India', 'UAE': 'UAE', 'US': 'USA', 'Unknown': 'USA'}
    region_totals = {}
    for row in region_rows:
        region = region_map.get(str(row[0]), str(row[0]))
        total_usd = float(row[3] or 0)
        region_totals[region] = total_usd

    # 3. Fetch daily signups (used as proxy for trial/user data)
    print("  Fetching GLP1 daily signups (card 20302)...")
    try:
        result = mb_post(f'card/{GLP1_DAILY_SIGNUPS_CARD}/query')
        signup_rows = result.get('data', {}).get('rows', [])
        print(f"    Got {len(signup_rows)} signup rows")
    except Exception as e:
        print(f"    ERROR fetching GLP1 signups: {e}")
        signup_rows = []
    ...
    for row in signup_rows:
        ...
        month_key = date_str[:7]
        if month_key not in glp1_signups:
            glp1_signups[month_key] = {'dates': [], 'signups': []}
        glp1_signups[month_key]['dates'].append(date_str)
        glp1_signups[month_key]['signups'].append(count)

    print(f"  GLP1 revenue months: {sorted(glp1_revenue.keys())}")
    print(f"  GLP1 region totals: {region_totals}")
    print(f"  GLP1 signup months: {sorted(glp1_signups.keys())}")

    return glp1_revenue, region_totals, glp1_signups


def merge_glp1_into_data(revenue_data, purchase_data, trial_data, country_revenue, glp1_revenue, glp1_region_totals, glp1_signups):
    """
    Merges GLP-1 data into the main data structures used by the dashboard.
    """
    print("  Merging GLP1 data into dashboard structures...")

    # Merge into revenue_data
    for month_key, mdata in glp1_revenue.items():
        if month_key not in revenue_data:
            revenue_data[month_key] = {
                'dates': mdata['dates'],
                'revenue': {p: [0]*len(mdata['dates']) for p in PLUGS},
                'subscriptions': {p: [0]*len(mdata['dates']) for p in PLUGS},
            }

        rd = revenue_data[month_key]
        # Ensure GLP1 key exists
        if 'GLP1' not in rd['revenue']:
            rd['revenue']['GLP1'] = [0] * len(rd['dates'])
            rd['subscriptions']['GLP1'] = [0] * len(rd['dates'])

        # Map GLP1 dates to existing date array
        date_to_idx = {d: i for i, d in enumerate(rd['dates'])}
        for i, d in enumerate(mdata['dates']):
            if d in date_to_idx:
                idx = date_to_idx[d]
                rd['revenue']['GLP1'][idx] = mdata['revenue'][i]
                rd['subscriptions']['GLP1'][idx] = mdata['orders'][i]
            elif d not in date_to_idx:
                # Date exists in GLP1 but not in main data — append
                rd['dates'].append(d)
                for p in PLUGS:
                    if p != 'GLP1':
                        rd['revenue'].setdefault(p, []).append(0)
                        rd['subscriptions'].setdefault(p, []).append(0)
                rd['revenue']['GLP1'].append(mdata['revenue'][i])
                rd['subscriptions']['GLP1'].append(mdata['orders'][i])

        # Sort by date after merge
        if rd['dates'] != sorted(rd['dates']):
            sorted_pairs = sorted(enumerate(rd['dates']), key=lambda x: x[1])
            new_indices = [p[0] for p in sorted_pairs]
            rd['dates'] = [rd['dates'][i] for i in new_indices]
            for p in rd['revenue']:
                rd['revenue'][p] = [rd['revenue'][p][i] if i < len(rd['revenue'][p]) else 0 for i in new_indices]
                rd['subscriptions'][p] = [rd['subscriptions'][p][i] if i < len(rd['subscriptions'][p]) else 0 for i in new_indices]

    # Merge into purchase_data
    for month_key, mdata in glp1_revenue.items():
        if month_key not in purchase_data:
            purchase_data[month_key] = {
                'dates': mdata['dates'],
                'purchases': {p: [0]*len(mdata['dates']) for p in PLUGS},
            }
        pd_m = purchase_data[month_key]
        if 'GLP1' not in pd_m['purchases']:
            pd_m['purchases']['GLP1'] = [0] * len(pd_m['dates'])
    ...
    for country in GLP1_ACTIVE_COUNTRIES:
        proportion = glp1_region_totals.get(country, 0) / total_region_rev

        if country not in country_revenue:
            country_revenue[country] = {}

        for month_key, mdata in glp1_revenue.items():
            if month_key not in country_revenue[country]:
                country_revenue[country][month_key] = {
                    'dates': mdata['dates'],
                    'revenue': {p: [0]*len(mdata['dates']) for p in PLUGS},
                    'subscriptions': {p: [0]*len(mdata['dates']) for p in PLUGS},
                }
            ...
            if 'GLP1' not in cr_m['revenue']:
                cr_m['revenue']['GLP1'] = [0] * len(cr_m['dates'])
                cr_m['subscriptions']['GLP1'] = [0] * len(cr_m['dates'])

            date_to_idx = {d: i for i, d in enumerate(cr_m['dates'])}
            for i, d in enumerate(mdata['dates']):
                if d in date_to_idx:
                    idx = date_to_idx[d]
                    cr_m['revenue']['GLP1'][idx] = round(mdata['revenue'][i] * proportion, 2)
